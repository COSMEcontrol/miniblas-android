...
public class Tools {
	public static Conexion getConexion(int sessionId, String sessionKey, HashMap<Integer,Session>  sessions) throws   SessionNotFound, IncorrectSessionKey{
...
//			application.
		Log.v("Abriendo base de datos", "Abriendo base de datos");
			Dao<MiniBlasPerfil,Integer> perfilDao = OpenHelperManager.getHelper(
					context, DBHelper.class).getPerfilDao();
			...
			if(profileList.size()>1){
				throw new ErrorProfile("ConnectionArcadioService ----> Error, encontrado "
						+ "mas de un perfil con ID pedido " + String.valueOf(id));
